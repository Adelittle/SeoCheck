...
$exec = curl_exec($curl);
curl_close($curl);
$alex1 = '<POPULARITY URL="(.*?)" TEXT="(.*?)"';
$alex2 = 'NAME="(.*?)" RANK="(.*?)"\/>';
preg_match_all("/$alex1/", $exec, $rank);
preg_match_all("/$alex2/", $exec, $country);
...
foreach ($urls as $domain) {
 ...
 $ale = getAlexa($domain);
 $moz = getMoz($domain);
 $tra = trafficCheck($domain);
 $result = ("$domain$ale$moz$tra");
 $save = @fopen("seo_result.txt", "a");
 fwrite($save, $result."\n");
 fclose($save);
}
